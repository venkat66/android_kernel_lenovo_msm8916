...
int fail_count = 0;//bug291249,shenyong.wt,20141001,add sdcard hangji protect
static void sdhci_data_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (host->data->error) {
		...
		sdhci_finish_data(host);
		/*bug291249,shenyong.wt,20101001,start,add sdcard hangji protect*/
		printk("XXX::err::fail_count=%d\r\n",fail_count);//hoper
		if(fail_count++ >5)
		{
			sdhci_reset(host, SDHCI_RESET_CMD);
			sdhci_reset(host, SDHCI_RESET_DATA);
		...
		fail_count = 0;
		/*bug291249,shenyong.wt,20101001,end,add sdcard hangji protect*/
